...
from Algorithm.GNNmodel.MAGNAKGE_model import MAGNAKGE
from Algorithm.learner.mhgnnedge_learner import MHGNNEDGELearner
...
class MHGNNEDGEAgent(BaseAgent):
    def __init__(self):
        configs_dict = get_configs("../algo_config/gnnedge_pd.yaml")
        config = argparse.Namespace(**configs_dict)
        
        super(MHGNNEDGEAgent, self).__init__(config)
        # Additional initialization for MHGNNEDGEAgent can be added here
    ...
    def _build_policy(self) -> Module:
        ...
        return MHGNNEdgeNetwork(config=self.config)

    def _build_learner(self, *args):
        return MHGNNEDGELearner(self.config, self.policy)
        ...
        if self.train_TA_model and random.uniform(0, 1) < self.alignEpsilon(self.step, sat):
            ...
            with torch.no_grad():
                if self.use_student_network:
                    ...
                    qValues = self.policy.sNetwork(newState, newState.ndata['1st_order_feat'], newState.edata['1st_order_weight'])
                else:
                    self.policy.qNetwork.eval()
                    # self.policy.qNetwork.g = newState
                    qValues = self.policy.qNetwork(newState, newState.ndata['feat'], newState.edata['weight'])
...
class MHGNNEdgeNetwork(Module):
    def __init__(self, config: Namespace):
        super(MHGNNEdgeNetwork, self).__init__()
        ...
        self.qNetwork = MAGNAKGE(
            num_layers=config.num_layers,
            in_ent_dim=config.input_ent_dim,
            in_rel_dim=config.input_edge_dim,
