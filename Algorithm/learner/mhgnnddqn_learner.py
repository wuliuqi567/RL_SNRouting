...
class MHGNNDDQNLearner(BaseLearner):
    def __init__(self, config: Namespace, policy: Module):
        ...
        self.scheduler = torch.optim.lr_scheduler.LinearLR(
            self.optimizer,
            start_factor=1.0,
            end_factor=getattr(config, 'end_factor', 0.2),
        ...
        self.student_scheduler = torch.optim.lr_scheduler.LinearLR(
            self.student_optimizer,
            start_factor=1.0,
            end_factor=getattr(config, 'student_end_factor', 0.1),
            total_iters=config.student_lr_decay_steps,
        )
