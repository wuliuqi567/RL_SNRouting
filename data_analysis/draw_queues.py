...
该模块用于从CSV文件中读取队列长度数据，并绘制多个算法的队列长度对比图。
主要功能：
- 自动发现和加载队列CSV文件
- 支持数据平滑处理（EMA或移动平均）
- 生成高质量的对比图表
...
def _load_queue_series(csv_path: PathLike) -> np.ndarray:
	"""
	从CSV文件中加载队列长度数据序列
	...
	返回:
		np.ndarray: 队列长度数据的numpy数组

	异常:
		ValueError: 如果队列数据为空
	"""
	# 读取CSV文件（队列文件每行一个数值，没有列名）
	df = pd.read_csv(csv_path, header=None, names=["queue"])
	# 转换为数值类型，无效值转为NaN，然后删除NaN值
	queue = pd.to_numeric(df["queue"], errors="coerce").dropna().to_numpy()

	if queue.size == 0:
		raise ValueError(f"Queue data is empty in: {csv_path}")
...
def discover_queue_csvs(
	...
	该函数会在root_dir下搜索每个算法的队列CSV文件。
	搜索策略：
	1. 如果指定了gateways，优先搜索精确匹配的文件（如Queues_4_Gateways.csv）
	2. 如果精确匹配失败，则搜索通配符模式（如Queues*Gateways.csv）
	...
	for algo in algorithms:
		...
		if gateways is not None:
			...
			patterns.append(f"**/{split}/csv/Queues_{gateways}_Gateways.csv")
		# 添加通配符模式作为备选
		patterns.append(f"**/{split}/csv/Queues*Gateways.csv")
...
def plot_queue_convergence(
	csv_files: Mapping[str, PathLike],
	save_path: Optional[PathLike] = None,
	title: str = "Queue Length Convergence",
	...
	for idx, (label, csv_path) in enumerate(csv_files.items()):
		try:
			# 加载队列数据
			queue = _load_queue_series(csv_path)
		...
		if max_points is not None and queue.size > max_points:
			step = int(np.ceil(queue.size / max_points))
			queue = queue[::step]

		# 对数据进行平滑处理
		smooth_queue = _smooth_series(queue, window=smooth_window, method=smooth_method)
		# 生成x轴坐标（时间步，从1开始）
		x = np.arange(1, len(queue) + 1)
		# 为每个算法分配颜色（循环使用配色方案）
		color = colors[idx % len(colors)]

		# 如果需要，绘制原始数据（半透明，细线）
		if show_raw:
			ax.plot(x, queue, color=color, alpha=0.16, linewidth=0.8)

		# 绘制平滑后的数据（不透明，粗线，带标签）
		ax.plot(x, smooth_queue, color=color, linewidth=2.2, label=label)

	# 设置图表标题和坐标轴标签
	ax.set_title(title, fontsize=13, fontweight="bold")
	ax.set_xlabel("Time Step", fontsize=11)
	ax.set_ylabel("Queue Length", fontsize=11)
...
def plot_from_simresults(
	...
	save_path: Optional[PathLike] = None,
	title: str = "Queue Length Comparison",
	smooth_window: int = 500,
	smooth_method: str = "ema",
	...
	if missing:
		print(f"[WARN] Queue csv not found for: {', '.join(missing)}")

	# 如果没有找到任何CSV文件，抛出异常
	if not csv_map:
		raise FileNotFoundError(
			f"No queue csv found under '{simresults_root}'. Check directory and pattern."
		)

	# 绘制对比图
	fig, ax = plot_queue_convergence(
		csv_files=csv_map,
		save_path=save_path,
		title=title,
...
if __name__ == "__main__":
	...
	# 设置输出图片路径（PDF格式）
	figure_path = repo_root / "data_analysis" / "queue_length_comparison.pdf"
	...
	fig, _, used_csvs = plot_from_simresults(
		...
		title="Queue Length Comparison (4 Gateways)",
